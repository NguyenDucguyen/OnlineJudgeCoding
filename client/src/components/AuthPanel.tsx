...
import { ChevronDown, ChevronUp, LogIn, LogOut, ShieldCheck, UserPlus } from 'lucide-react';
...
const AuthPanel: React.FC = () => {
  ...
  return (
    <div className="fixed top-24 right-4 z-40 w-full max-w-sm space-y-2">
      <div className="flex justify-end">
        <button
          type="button"
          onClick={() => setIsCollapsed((prev) => !prev)}
          className="inline-flex items-center space-x-2 px-3 py-2 text-sm font-medium bg-gray-900 text-white rounded-md shadow-md hover:bg-gray-800"
          aria-expanded={!isCollapsed}
          aria-controls="auth-panel"
        >
          {isCollapsed ? <ChevronDown className="w-4 h-4" /> : <ChevronUp className="w-4 h-4" />}
          <span>{isCollapsed ? 'Mở đăng nhập/đăng kí' : 'Thu gọn'}</span>
        </button>
      </div>
      ...
      {!isCollapsed && (
        ...
        >
          <div className="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
            <div>
              <h3 className="text-lg font-semibold text-gray-900">Đăng nhập/Đăng kí</h3>
              <p className="text-sm text-gray-600">Cần đăng nhập để nộp bài và xem lịch sử.</p>
            </div>
            <div className="flex space-x-2" role="group" aria-label="Authentication mode">
              <button
                onClick={() => setMode('login')}
                className={`inline-flex items-center space-x-2 px-3 py-2 rounded-md text-xs font-medium border ${
                  mode === 'login'
          ...
          <div className="p-4 space-y-4">
            {user ? (
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 flex items-start justify-between">
                ...
                <button
                  onClick={logout}
                  className="inline-flex items-center space-x-2 px-3 py-2 text-sm font-medium text-red-600 bg-red-50 hover:bg-red-100 rounded-md"
                >
